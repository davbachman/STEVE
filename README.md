...
5. Constants default to `Continuous`, which matches the original single-slider behavior and evaluates the plot at one parameter value.
...
8. Navigate the viewport with right-drag to orbit and `Shift` + right-drag to pan. Drag selected objects with left-drag, and use `Shift` + left-drag to constrain object dragging along Z.
